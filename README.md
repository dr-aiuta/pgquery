...
- **node-pg-migrate runs the migrations.** Committed migration files stay frozen history and are applied the same way everywhere.
...
Edit your table definitions, then generate a draft migration against a development database that is already migrated:

```typescript
// scripts/generate-migration.ts
import {writeFileSync} from 'fs';
import {Client} from 'pg';
import {generateMigration} from 'pg-lightquery';
import {usersTable, postsTable} from '../src/db/tables';

const client = new Client({connectionString: process.env.DATABASE_URL});
await client.connect();
const draft = await generateMigration([usersTable, postsTable], {
	query: (text, values) => client.query(text, values),
	format: 'node-pg-migrate-ts', // or 'node-pg-migrate-js' or 'sql'
});
await client.end();
...
Review the file, run `node-pg-migrate up`, and commit it. The output looks like this:
...
export async function up(pgm: MigrationBuilder): Promise<void> {
	...
	// REVIEW: Dropping legacy_bio deletes its data. If it was renamed, use: ALTER TABLE "public"."users" RENAME COLUMN "legacy_bio" TO "bio"
	// pgm.sql(`ALTER TABLE "public"."users" DROP COLUMN "legacy_bio"`);

	// No down migration is generated. Add one if you need rollbacks.
}
```

The generator follows one safety rule:
...
### Running migrations, then checking drift at startup
